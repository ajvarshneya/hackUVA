...
public class Image extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        max = getResources().getInteger(R.integer.max_rams);

        Random generator = new Random();
        int rand = generator.nextInt(max) + 1;
        loadImage("ram" + rand);

        ramLabel = (TextView)findViewById(R.id.tv_image);

        ramLabel.setText("Ram " + rand);
    ...
    public void loadImage(String id) {
        Picasso.with(this).load("http://swaha.me/images/" + id + ".jpg").into(iv);
    }

    public void onTouchRam(View view) {
        Random generator = new Random();
        int rand = generator.nextInt(max) + 1;
        loadImage("ram" + rand);
        ramLabel.setText("Ram " + rand);
